onnx_path=$1 # yolov8s onnx model path, recommend using the final opt model generated by tpu-mlir
onnx_input_name=$2 # yolov8s input name, use scripts/get_onnx_input_output.py to get it
...
calib_table=$6 # calib table path of yolov8s onnx, use tpu-mlir to generate it
